...
lengthV (V a b c) = sqrt (a*a + b*b + c*c)

normalizeV :: Vector -> Vector
normalizeV (V a b c) = (V (a / (lengthV (V a b c))) (b / (lengthV (V a b c))) (c / (lengthV (V a b c))))
...
{-
 2. (4p) Liste imbricate
  ...
  (1p) flatten
-}
data SList a = UndefinedSList | List [SList a] | Elem a deriving Show
...
deepEqual (List xs) (List ys) = if length xs == length ys then and $ zipWith deepEqual xs ys else False
...
-}
data BST a = UndefinedNode | BSTNil | BNode a (BST a) (BST a) deriving Show

insertElem :: (Ord a, Eq a) => BST a -> a -> BST a
insertElem BSTNil a = (BNode a BSTNil BSTNil)
insertElem (BNode root left right) x = if x <= root 
  then BNode root (insertElem left x) right
  else BNode root left (insertElem right x)

findElem :: (Ord a, Eq a) => BST a -> a -> Maybe a
findElem BSTNil x = Nothing
findElem (BNode root left right) x = if x == root
  then Just x
  else if x > root
    then findElem right x
    else findElem left x

subTree :: (Ord a, Eq a) => BST a -> a -> a -> Maybe (BST a)
subTree (BNode root left right) x y
  | findElem (BNode root left right) x == Nothing || findElem (BNode root left right) y == Nothing = Nothing
  | x <= root && y <= root = subTree left x y
  | x > root && y > root   = subTree right x y
  | otherwise = Just (BNode root left right)


inorder :: BST a -> [a]
inorder (BNode x BSTNil BSTNil) = [x]
inorder (BNode root left right) = inorder left ++ [root] ++ inorder right
