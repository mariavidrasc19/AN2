...
(define (compute-length get-line-segment get-start-point get-end-point)
  (let* ((segment (get-line-segment))
         (inceput (get-start-point segment))
         (final (get-end-point segment))
         (inceput_x (car inceput))
         (inceput_y (cdr inceput))
         (final_x (car final))
         (final_y (cdr final)))
    (let ((x (- final_x inceput_x)) (y (- final_y inceput_y))) (sqrt (+ (* x x) (* y y))) ) ))
...
(define (distance p1 p2)
  (let* ((segment (cons p1 p2)) (get-segment (lambda() segment)) (start (lambda(segment) p1)) (final (lambda(segment) p2)))
    (compute-length get-segment start final)) )
...
(define (compute-perimeter points)
    (let perimetru ((point (car points)) (points points))
      (if (<= (length points) 1)
          (if (= (length points) 0)
              0
              (+ (distance (car points) point) (perimetru point '())))
          (+ (distance (car points) (cadr points)) (perimetru point (cdr points)) )) ) )
...
(define (3-sequence-max numbers separator)
  (let*-values ( ((f) (lambda(number) (not (= number separator))))
                 ((a b) (splitf-at numbers f))
                 ((b c) (splitf-at (cdr b) f))
                 ((c d) (splitf-at (cdr c) f)) )
    (max (sum b) (max (sum a) (sum c))) ))
...
(define (list-num-concat numbers)
  (let loop ((n (cdr numbers)) (concat (car numbers)))
    (if (null? n)
        concat
        (loop (cdr n) (string->number (string-append (number->string concat) (number->string (car n))))))))
...
(define (find-all-suffixes number)
    (let loop ((listutza (number->list number)) (rez '()))
          (if (null? listutza)
              (reverse rez)
              (loop (cdr listutza) (cons (list-num-concat listutza) rez) )) ))
...
(define (run initial-state final-state next)
    (let iter ((a initial-state) (b final-state) (result '()))
      (if (= a b)                              
        (reverse (cons a result))
        (iter (next a) b (cons a result)))))
...
(define (generate-number k x)
  (let iter ((lista (run 0 (- k 1) add1)) (numar 0))
    (if (null? lista)
        numar
        (iter (cdr lista) (+ (* numar 10) (+ x (* k (car lista)))) ))))
