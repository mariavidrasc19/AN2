...
myConcat([],L,L).
myConcat([First|Rest1], L, [First|Rest2]):- myConcat(Rest1, L, Rest2).
...
myReverse([],[]).
myReverse([Head|List], ReverseList):- myReverse(List, ReverseList1), myConcat(ReverseList1, [Head], ReverseList).
...
myReverseAcc([], L, L).
myReverseAcc([Head|List1], List2, ReverseAcc):- myReverseAcc(List1, [Head|List2], ReverseAcc).
...
extract([], _, _, []).
extract([El:_], 0, 0, [El]).
extract([El | List1], 0, End, [El | List2]):- End1 is End - 1, extract(List1, 0, End1, List2).
extract([_|List1], Start, End, List2):- Start1 is Start - 1, End1 is End - 1, extract(List1, Start1, End1, List2).
...
factorial(0, 1).
factorial(N, Fact) :- factorial(N1, Fact1), N is N1 + 1, Fact is Fact1 * N.
...
sublista([],[]).
%%sublista([_], [_]).
sublista([_ | List], Sublist) :- sublista(List, Sublist).
sublista([El | List], [El | Sublist]) :- sublista(List, Sublist).
...
descendants(Nod, Nr) :- arc(Nod, Copil), descendants(Copil, Nr1), Nr is Nr1 + 1. 
descendants(Nod, Nr) :- arc(Nod, Copil1), (arc(Nod, Copil2), Copil1 \= Copil2), descendants(Copil1, N1), descendants(Copil2, N2), Nr is N1 + N2 + 2.
