...
class (Ord a) => PQueue pq a where
    ...
    toList :: pq a -> [(Prio, a)]
    toList pQueue = if (isEmpty pQueue) then [] else (fromJust (top pQueue)) : (toList (pop pQueue))

    size :: pq a -> Int
    size el = length (toList el)
...
instance (Ord a) => PQueue ListPQ a where
    ...
    insert new (LPQ list) = if list == [] then LPQ [new] 
                                         else if fst new >= fst (head list) then LPQ (new : list)
                                                                        else LPQ $ head list : toList (insert new $ LPQ $ tail list)

    top (LPQ list) = if list == [] then Nothing else Just (head list)

    pop (LPQ list) = LPQ (tail list) 

    isEmpty (LPQ list) = list == []
...
merge tree1@(Node r1 (p1, val1) left1 right1) tree2@(Node r2 (p2, val2) left2 right2) = if p1 <= p2 then merge tree2 tree1
                                else if rank left1 >= rank (merge right1 tree2) then (Node (1 + rank (merge right1 tree2)) (p1, val1) left1 (merge right1 tree2))
                                                                      else (Node (1 + rank left1) (p1, val1) (merge right1 tree2) left1)

inorder :: LeftistPQ a -> [(Prio, a)]
inorder (Empty _) = []
inorder (Node r node left right) = inorder left ++ [node] ++ inorder right
...
instance (Ord a) => PQueue LeftistPQ a where
    ...
    top list = if isEmpty list then Nothing else Just (nodeVal list)

    pop list = merge (left list) (right list)
...
convert list = fromList . toList $ list 
