...
arithmetic initial r = iterate (+ r) initial
geometric :: Num a => a -> a -> [a]
geometric initial q = iterate (* q) initial
{-
  3. (2p)
...
randoms :: Int -> [Word8]
randoms seed = map (fromIntegral . fst) . iterate (next . snd) $ el
  where el = next . mkStdGen $ seed
{-
    4. (1p)
...
wordToAlpha x = chr $ ord 'a' + (fromIntegral $ mod x 26)
...
randomAlphaKey init = map wordToAlpha (randoms init) 
{-
  6. (3p)
...
tableToFunc rot13Table x = snd (head (filter (\p -> fst p == x) rot13Table))
        
substCrypt :: [(Char, Char)] -> String -> String
substCrypt rot13Table str = map (tableToFunc rot13Table) str
{-
    7. (Bonus - 1p)
