...
(define (all-palindromes? n Bases)
  (if (null? Bases)
      #t
      (if (palindrome? (num->base n (first Bases)))
...
(define (if-b-pal counter start Bases b)
  (if (null? Bases)
      (if (>= counter b)
      #t
      #f)
      #f)
  (if (all-palindromes? start (list (first Bases)))
      (+ counter 1)
      (if-b-pal counter start (cdr Bases) b) ))

(define (first-b-pal start b)
  (if (if-b-pal 0 start '(2 3 4 5 6 7 8 9 10) b)
      start
