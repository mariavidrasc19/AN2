...
-}
primes :: Int -> [Int]
primes n = [ x | x <- [0 .. n], prime x == [1, x]]
      where prime x = [y | y <- [1 .. x], x `mod` y == 0]
...
group2 [] = []
group2 [x] = [[x]]
group2 (x:y:ys) = let (first:rest)  = group2 (y:ys)
                  in  if x /= y
                      then [x]:first:rest
                      else (x:first):rest
...
-}
nrOcc :: Ord a => [a] -> [(a, Int)]
parse1 x = ((head x), (length x))
nrOcc x = map parse1 (group2 (sort x))
...
-}
dup :: String -> String
dup list = unwords $ map (\w -> w ++ " " ++ w) $ words list
...
isIsogram s = True
-- isIsogram = all (==1) filter (\x -> (snd x) == 1) . nrOcc
